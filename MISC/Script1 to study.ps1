...
#------------------------------------------------------------------------
Param($InstallFile, [switch] $Uninstall, [switch] $UninstallCurrent, [switch] $Install, [switch] $Repair, [switch] $debug, [switch] $restart)
...
# $version = "02.044" #2014-07-28 - KR - Made replacement values more unique by appending asterisks at beginning and end of items to be replaced in XML file.
$version = "02.046" #2014-07-28 - KR - Added search and replace for ValidateValue and WaitValue; Fixed issue with return codes so that they are added to Global Array; Fixed issue with trying to write to log file if XML file doesn't exist
#------------------------------ SET-UP GUI ------------------------------
Clear-Host
#-------------------------- RETURN CODE VALUES --------------------------
...
#- 26001	-	$GLOBAL:arrRC is null
#- 26002	-	$GLOBAL:arrRC does not meet any other criteria
#------------------------- Variables -------------------------
...
Function fSTDOUT($text, $Color) {
	...
	IF ($text -eq "") {
		Write-Host
		IF (!($strLog -eq $null)){" " | Out-File $LOGFILE -Append}
	} ELSE {	
		...
		IF (!($strLog -eq $null)){"$DateNow - $text" | Out-File $LOGFILE -Append}
	}	
}
#------------------------- Get XML Attributes -------------------------
Function fGetXmlAtt($XmlPath, $XmlAttribute) {
	Try {
		...
		fSTDOUT "Fatal Exception: Getting xml attribute, $XmlAttribute - $_" 3 ; $GLOBAL:arrRC += 14041 ; fFindReturnCodes $GLOBAL:arrRC
	}
}
...
Function fCheckIdb {
	If(-Not(Test-Path "$strMyDir\Install.xml")) { fSTDOUT "Fatal Exception: Install.xml could not be found" 3 ; $GLOBAL:arrRC += 22000 ; fFindReturnCodes $GLOBAL:arrRC }
}
...
Function fCheckAdminRights {
	...
	If (-Not ([Security.Principal.WindowsPrincipal] [Security.Principal.WindowsIdentity]::GetCurrent()).IsInRole([Security.Principal.WindowsBuiltInRole] "Administrator")) { 
        fSTDOUT "Error: $strUSERNAME does not have administrative rights" 3; $GLOBAL:arrRC += 8344; fFindReturnCodes $GLOBAL:arrRC
...
Function fUninst ($strGUID) {
	...
	IF (($err9 -eq 0) -OR ($err9 -eq 3010) -OR ($err9 -eq 1641)) {
		...
		fSTDOUT "Error: Uninstall of $strGUID FAILED" 3
...
Function fParseXMLSections($XmlPath, $Task) {
	Try {
		...
		ForEach ($XmlNode In $XmlNodes) {
			...
			$ValidateValue = $ValidateValue -replace "\*RFC\*", "$intRFC"
			...
			$WaitValue = $WaitValue -replace "\*RFC\*", "$intRFC"
				...
				IF (($strProdWanted -eq $strProdType) -OR ($strProdWanted -eq "B") -OR ($strProdWanted -eq "")){
					...
					FOREACH ($num in $arrPlatforms){
						...
						IF ($num.contains("+")){
							...
							IF($intVer -ge $num){$bValidOS=$True} ELSE {$bValidOS=$False ; $logtext = "Skipping $Name $Task (Un-supported OS, minimum OS version $num)..."}
						}ELSE{
							# Perform Absolute OS Check - Is the current platform of the system equal to the OS listed in the XML
							IF($intVer -eq $num){$bValidOS=$True} ELSE {$bValidOS=$False ; $logtext = "Skipping $Name $Task (Does not apply to $num)..."}
						}					
					}
					IF($bValidOS) {
					#Validate the validation entry
					If(((fCheckVal $ValidateType $ValidateValue $ValidationBoolean) -eq $ValidationBoolean)) {
						fSTDOUT "Performing $Task of $Name"
	...
	Catch { fSTDOUT "Fatal Exception: Processing $Name $Task  $_" 3 ; $GLOBAL:arrRC += 24001 ; fFindReturnCodes $GLOBAL:arrRC }
}
...
Function fWriteChgInfoReg ($strDept, $strAgency, $strBranch, $intRFC) {
	IF ($dAdminCheck){
		...
		Try {
			...
			fSTDOUT "Fatal Exception: Adding Change Info to Registry -Error: $_" 3 ; $GLOBAL:arrRC += 1013 ; fFindReturnCodes $GLOBAL:arrRC 
		}
...
Function fRemoveChgInfoReg ($strDept, $strAgency, $strBranch, $intRFC) {
	IF ($dAdminCheck){
		Try {
			...
			fSTDOUT "Fatal Exception: Removing Change Info from Registry -Error: $_" 3 ; $GLOBAL:arrRC += 1011 ; fFindReturnCodes $GLOBAL:arrRC	
		}
...
Function fWriteRegValue($KeyPath, $Key, $Value, $Type) {
	Try {
		...
		fSTDOUT "Fatal Exception: $_" 3 ; $GLOBAL:arrRC += 1013 ; fFindReturnCodes $GLOBAL:arrRC   
	}
}
#------------------------- Check Success Codes -------------------------
...
Function fStartWait($WaitType, $WaitValue, $TimeOut) {
	...
	}Catch { fSTDOUT "Wait Failed: Processing $WaitValue - $_" 3 ; $GLOBAL:arrRC += 24061 ; fFindReturnCodes $GLOBAL:arrRC }
}
#------------------------- Find registry value -------------------------
...
Function fFindReturnCodes($GLOBAL:arrRC) {
	...
	ELSEIF ($GLOBAL:arrRC -Contains "3010"){
		IF ($debug) {fSTDOUT "$tab 3010 was found within return code array the system will have the bypass setup so that i can be restarted by the user."}
	...
	ELSEIF ($GLOBAL:arrRC -eq $null) {
                                IF ($debug) {fSTDOUT "$tab 26001 Success codes are Null"}
                                $intRC = 26001;fClose $intRC
		...
		IF ($debug) {fSTDOUT "$tab 26002 Could not return error code"}
                $intRC = 26002
                fClose $intC		
	}
}
...
$strPkgCache = "C:\$strDept\$strAgency\$strBranch\Cache"
$strLog = "C:\$strDept\$strAgency\$strBranch\Logs"
If(-Not (Test-Path "$strLog")) { Md "$strLog" -Force > $Null }
